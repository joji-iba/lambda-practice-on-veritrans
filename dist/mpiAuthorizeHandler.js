...
// テスト環境エンドポイント
const VERITRANS_MPI_ENDPOINT = "https://api3.veritrans.co.jp/test-paynow/v2/Authorize/mpi";
/**
 * authHash を生成する
 * authHash = SHA256(merchantCcid + minify(params) + merchantKey)
 */
const generateAuthHash = (merchantCcid, params, merchantKey) => {
    // params を最小化（minify）
    const minifyParams = JSON.stringify(params);
    // 連結して SHA256 ハッシュを生成
    const joinedString = merchantCcid + minifyParams + merchantKey;
    return (0, crypto_1.createHash)("sha256").update(joinedString, "utf8").digest("hex");
};
/**
 * 一意のオーダーIDを生成
 */
...
const handler = async (event) => {
    try {
        ...
        console.log("MPI Authorize Request:", JSON.stringify({
            orderId: orderId,
            amount: body.amount,
        ...
        console.log("MPI Authorize Response:", JSON.stringify({
